...
class _SubjectAttendanceDetailScreenState
    ...
    return Scaffold(
      ...
      body: attendanceAsync.when(
        ...
        data: (records) {
          ...
          final statuses = ref
              .read(getSubjectCalendarStatusesProvider)
              .call(
                schedulesForSubject: schedules,
                recordsForSubject: records,
                month: _visibleMonth,
  ...
  Future<void> _showMarkSheet(DateTime date, List schedules) async {
    final matching = schedules
        .where((s) => s.weekday == date.weekday && s.isActive)
        .toList();
        ...
        child: Wrap(
          children: [
            ...
            ListTile(
              ...
              onTap: () => Navigator.pop(context, AttendanceStatus.cancelled),
            ),
            ListTile(
              leading: Icon(Icons.event_busy, color: Colors.grey),
              title: Text('No Class'),
              onTap: () => Navigator.pop(context, AttendanceStatus.noClass),
            ),
          ],
        ),
      ),
